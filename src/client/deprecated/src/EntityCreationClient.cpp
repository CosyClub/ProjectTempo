                       ...
                       tempo::SystemLevelManager system_gm)
{
	...
	case EID_PLAYER: {
		...
		return_entity = newPlayer(world, scene, instance_id, type_id, p.localPlayer, pos.x, pos.y,
		                          p.health, system_gm);
		break;
	}
	case EID_AI: {
		AI_t a        = data.entity_type.ai;
		return_entity = newAI(world, scene, instance_id, type_id, pos.x, pos.y);
		break;
	}
	case EID_DES: {
		Destroyable_t d = data.entity_type.destroyable;
		return_entity   = newDestroyable(world, scene, instance_id, type_id, pos.x, pos.y,
	...
	case EID_NONDES: {
		...
		return_entity      = newNonDestroyable(world, scene, instance_id, type_id, pos.x, pos.y,
                            ...
                            std::string         mesh_name)
{
	...
	entity_object.addComponent<tempo::ComponentRender>(scene, mesh_name)
	  .node->attachObject(entity_mesh);
