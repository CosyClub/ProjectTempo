...
core::array<s32> CBatchingMesh::addMesh(IMesh *         mesh,
                                        core::vector3df pos,
                                        core::vector3df rot,
...
s32 CBatchingMesh::addMeshBuffer(IMeshBuffer *   buffer,
                                 core::vector3df pos,
                                 core::vector3df rot,
                                 core::vector3df scale)
{
...
s32 CBatchingMesh::addMeshBuffer(IMeshBuffer *buffer, const core::matrix4 &transform)
{
	...
	for (i = 0; i < MaterialReferences.size(); ++i) {
		if (MaterialReferences[i].VertexType == vt && MaterialReferences[i].Material == m) {
			// will there be too many vertices in the buffer?
			u32 newTotalI =
			  buffer->getIndexCount() + DestBuffers[MaterialReferences[i].BufferIndex].IndexCount;
			u32 newTotalV =
			  buffer->getVertexCount() + DestBuffers[MaterialReferences[i].BufferIndex].VertexCount;
...
//! Moves a mesh
core::array<bool> CBatchingMesh::moveMesh(const core::array<s32> &bufferIDs,
...
void CBatchingMesh::updateDestFromSourceBuffer(u32 i)
{
	...
	case video::EVT_2TCOORDS: {
		SMeshBufferLightMap *dest =
		  (SMeshBufferLightMap *) DestBuffers[BufferReferences[i].DestReference].Buffer;
	...
	case video::EVT_TANGENTS: {
		SMeshBufferTangents *dest =
		  (SMeshBufferTangents *) DestBuffers[BufferReferences[i].DestReference].Buffer;
