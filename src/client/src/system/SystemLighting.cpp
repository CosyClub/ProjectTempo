...
namespace client {
	...
	void SystemLighting::setup(irr::scene::ISceneManager *smgr,
		...
		this->light_ML1 = smgr->addLightSceneNode(
			parent, 
			irr::core::vector3df(0, 5.0f, 0), 
			irr::video::SColorf(0.1, 0.1, 0.1, 0.0), 
			15);
		//this->light_ML1->enableCastShadow(true);
		//this->light_ML1->setVisible(true);

		this->data_ML1 = this->light_ML1->getLightData();
	...
	void SystemLighting::update(irr::video::SColor colour) {
		irr::core::vector3df c = client::RGBtoHSV(colour);
		c.Z *= 0.3;
		irr::video::SColor dc = client::HSVtoRGB(c);
		...
		this->data_ML1.DiffuseColor = dc;//irr::video::SColorf(red, green, blue, 0.0);
