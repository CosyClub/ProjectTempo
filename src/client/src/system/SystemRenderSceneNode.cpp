...
void SystemRenderSceneNode::setup(irr::scene::ISceneManager *smgr) {
  ...
  for (auto entity : entities) {
    ...
    // a billboard has its origin in the centre, this node is used for
    // alignement
    sn.node = smgr->addEmptySceneNode();
    sn.node->setPosition(irr::core::vector3df(0.0f, 0.0f, 0.0f));

	irr::scene::IBillboardSceneNode *billboard = new irr::scene::YAlignedBillboardSceneNode(
		sn.node, smgr, -1, pos, size, top_color, bottom_color);
  }
...
void SystemRenderSceneNode::update() {
  ...
  for (auto entity : entities) {
    ...
    tempo::ComponentStagePosition &sp =
    ...
    // std::cout << "Moved billboard to: " << pos.x << ", " << s.getHeight(pos)
    // << ", " << pos.y << std::endl;
