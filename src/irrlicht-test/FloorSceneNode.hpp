...
class FloorSceneNode : public irr::scene::ISceneNode
{
   ...
   public:
	FloorSceneNode(irr::scene::ISceneNode *   parent,
	...
	void updateTiles(float *heights)
	{
		...
		for (int x = 0; x < this->size.Width; ++x) {
			for (int y = 0; y < this->size.Height; ++y) {
				...
				vertices[i * 4 + 0] =
				  irr::video::S3DVertex(x + 0, heights[i], y + 0, 0, 1, 0, white, 0, 0);
				vertices[i * 4 + 1] =
				  irr::video::S3DVertex(x + 1, heights[i], y + 0, 0, 1, 0, white, 1, 0);
				vertices[i * 4 + 2] =
				  irr::video::S3DVertex(x + 0, heights[i], y + 1, 0, 1, 0, white, 0, 1);
				vertices[i * 4 + 3] =
				  irr::video::S3DVertex(x + 1, heights[i], y + 1, 0, 1, 0, white, 1, 1);
