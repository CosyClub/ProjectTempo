...
enum ClientRole { NO_ROLE, PLAYER, SPECTATOR };
...
enum HandshakeID { DEFAULT, HELLO, HELLO_ROG, ROLEREQ, ROLEREQ_ROG };
...
#define tcpRemoteToStr(sock)                                                                       \
	(sock->getRemoteAddress().toString() + ":" + std::to_string(client->getRemotePort()))
